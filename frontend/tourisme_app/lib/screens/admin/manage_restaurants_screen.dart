...
class ManageRestaurantsScreen extends StatefulWidget {
  ...
  const ManageRestaurantsScreen({super.key, required this.token});
...
class _ManageRestaurantsScreenState extends State<ManageRestaurantsScreen>
  ...
  void initState() {
    super.initState();
    restaurantsFuture = RestaurantService().getRestaurants(widget.token);
  ...
  Future<void> _confirmDelete(Restaurant restaurant) async {
    ...
    if (confirmed == true && mounted) {
      try {
        await RestaurantService()
            .deleteRestaurant(restaurant.id, widget.token);
        _refresh();
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("« ${restaurant.name} » supprimé avec succès"),
                        ...
                        const Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 5),
                              Text(
                                "Gérez votre catalogue de restaurants",
                                style: TextStyle(
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          return _RestaurantCard(
                            ...
                            onDelete: () =>
                                _confirmDelete(restaurants[index]),
                          );
                        },
                      );
                    },
