...
import 'package:tourisme_app/screens/admin/edit_beach_screen.dart';
import '../../models/beach.dart';
import '../../services/beach_service.dart';

class ManageBeachesScreen extends StatefulWidget {
  final String token;

  const ManageBeachesScreen({super.key, required this.token});

  @override
  State<ManageBeachesScreen> createState() => _ManageBeachesScreenState();
}

class _ManageBeachesScreenState extends State<ManageBeachesScreen>
  ...
  void initState() {
    super.initState();
    beachesFuture = BeachService().getBeaches(widget.token);
  ...
  Future<void> _confirmDelete(Beach beach) async {
    ...
    if (confirmed == true && mounted) {
      try {
        await BeachService().deleteBeach(beach.id, widget.token);
        _refresh();
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text("« ${beach.name} » supprimée avec succès"),
                        ...
                        const Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: 5),
                              Text(
                                "Gérez votre catalogue de plages",
                                style: TextStyle(
                        ...
                        child: Row(
                          ...
                          children: [
                            Icon(Icons.waves_rounded,
                                color: Colors.blue.shade400, size: 12),
                            const SizedBox(width: 3),
                            Text(
                              "Baignade",
                              style: TextStyle(
