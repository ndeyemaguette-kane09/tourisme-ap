...
class AddBeachScreen extends StatefulWidget {
  final String token;
  const AddBeachScreen({super.key, required this.token});
...
class _AddBeachScreenState extends State<AddBeachScreen>
  ...
  void addBeach() async {
    ...
    try {
      ...
      await ApiService.addBeach(widget.token, {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text("🏖️ Plage ajoutée avec succès !"),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text("Erreur lors de l'ajout de la plage"),
            backgroundColor: const Color(0xFFB71C1C),
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          ),
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: _isLoading ? null : addBeach,
                              style: ElevatedButton.styleFrom(
                                backgroundColor: const Color(0xFFBF360C),
                                foregroundColor: Colors.white,
                                elevation: 5,
                                shadowColor: const Color(0xFFBF360C).withOpacity(0.4),
