...
class EditBeachScreen extends StatefulWidget {
  final Beach beach;
  final String token;

  const EditBeachScreen({
    super.key,
    required this.beach,
    required this.token,
  });
  ...
  @override
  State<EditBeachScreen> createState() => _EditBeachScreenState();
}

class _EditBeachScreenState extends State<EditBeachScreen>
  ...
  void initState() {
    ...
    nameController = TextEditingController(text: widget.beach.name);
    cityController = TextEditingController(text: widget.beach.city ?? "");
    imageController = TextEditingController(text: widget.beach.imageUrl ?? '');
    descriptionController =
        TextEditingController(text: widget.beach.description ?? "");
    addressController = TextEditingController(text: widget.beach.address ?? "");
  ...
  // ── LOGIQUE INCHANGÉE ──
  Future<void> _updateBeach() async {
    ...
    try {
      await BeachService().updateBeach(
        widget.beach.id,
        {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text("Plage modifiée avec succès"),
                          ...
                          const Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text("la plage", style: TextStyle(color: Colors.white, fontSize: 26, fontWeight: FontWeight.w900, height: 1.1, letterSpacing: -0.5)),
                                SizedBox(height: 5),
                        ...
                        children: [
                          ...
                          Row(children: [
                            ...
                            Padding(
                              ...
                              child: Icon(Icons.waves_rounded, color: Colors.grey.shade300, size: 16),
                            ),
                            Expanded(child: Divider(color: Colors.grey.shade200, thickness: 1)),
                          ]),
              ...
              child: Container(
                ...
                child: Container(
                  ...
                  child: Material(
                    ...
                    child: InkWell(
                      borderRadius: BorderRadius.circular(18),
                      onTap: isLoading ? null : _updateBeach,
                      child: Center(
                        child: isLoading
